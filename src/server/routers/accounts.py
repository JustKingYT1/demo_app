...
@accounts_router.get(path='/{user_id}', response_model=dict)
def get_account(user_id: int) -> dict:
    return accounts.get(user_id=user_id)


@accounts_router.get(path='/', response_model=dict)
def get_all_accounts() -> dict:
    return accounts.get_all()


@accounts_router.post(path='/', response_model=dict)
def new_account(account: Accounts) -> dict:
    return accounts.new(account=account)


@accounts_router.put(path='/updateLogin/{user_id}', response_model=dict)
...
@accounts_router.delete(path='/{user_id}', response_model=dict)
